...
namespace UnityTool.ComponentUtils
{
    public static class TransformUtil
    {
        ...
        public static bool TryGetComponentInChildren<T>(this Transform parent, out T param)
        {
            if (parent.TryGetComponent<T>(out param))
            {
                return true;
            }
            else
            {
                if (parent.childCount > 0)
                {
                    ...
                    {
                        if (TryGetComponentInChildren<T>(parent.GetChild(i), out param))
                            return true;
                    }
                }
                return false;
